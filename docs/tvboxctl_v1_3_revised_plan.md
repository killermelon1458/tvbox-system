# TVBoxCtl V1.3 Revised Implementation Plan
...
V1.3 updates the older Chromium-app concept into a browser-agnostic webapp framework. Webapps are still deterministic app targets: each app definition pins its browser, URL, profile, icon, and input profile. TVBox should not silently switch browsers just because another browser is installed.
...
V1.3 allows only one TVBox-local foreground app/session at a time.
...
This is intentionally simpler than keeping one background app alive. The Pi should not accumulate YouTube, Steam Link, webapp games, Spotify UI, and Kodi all at once.
...
V1.3 should clear or ignore `preserved-app`.
...
moonlight:desktop
spotify
webapp:youtube-chromium
webapp:youtube-brave
webapp:fireboy-watergirl
webapp:fixitfelix
desktop
unknown
```
...
```text
context=webapp:youtube-chromium
input_profile=youtube_remote

context=webapp:youtube-brave
input_profile=youtube_remote

context=webapp:generic-tv-site
input_profile=controller_remote_clone

context=webapp:fireboy-watergirl
...
Do not launch Kodi, browsers/webapps, Steam Link, Moonlight, or other long-running apps while they still hold the `tvboxctl` lock FD.
...
V1.3 should use a central cleanup function.
...
close_local_apps_except(target):
  ...
  close local webapps unless target is that exact webapp
...
```text
Webapps are first-class local external apps.
V1.3 does not preserve local webapps behind Kodi.
Launching one webapp closes other local webapps.
A webapp is closed by matching its configured profile path, not by broad-killing the browser.
```
...
## 6.5 close_webapp(app_id)

Webapps must use unique dedicated browser profile directories.

Example profiles:

```bash
/home/tvbox/.config/chromium-tvbox-youtube
/home/tvbox/.config/brave-tvbox-youtube
/home/tvbox/.config/chromium-tvbox-fireboy-watergirl
/home/tvbox/.config/chromium-tvbox-fixitfelix
```

Close logic should read the webapp definition, get its configured `profile=...`, and close only browser processes using that profile path.

Required behavior:

```text
close_webapp(app_id):
  read /opt/tvbox-system/config/webapps/<app_id>.conf
  extract profile path
  close windows/processes matching that profile path
  wait briefly
  force-kill only matching profile-path processes if graceful close fails
```

Forbidden behavior:

```text
Do not use broad pkill chromium.
Do not use broad pkill brave.
Do not use broad pkill edge.
Do not kill unrelated browser sessions.
```

## 6.6 close_all_webapps_except(app_id)

The generic cleanup path should be able to close all configured local webapps except the target being launched.

Concept:

```text
For every config file in /opt/tvbox-system/config/webapps/*.conf:
  if config.app_id != target_app_id:
    close_webapp(config.app_id)
```

Reason:

```text
YouTube Chromium, YouTube Brave, Fireboy and Watergirl, Fix-It Felix, and future browser-backed apps are all local external apps.
V1.3 should not allow multiple local webapps to accumulate behind Kodi.
```
...
Webapps should be generic, browser-agnostic app targets, not one-off scripts forever.

The important abstraction is:

```text
browser-backed webapp
```
...
A webapp may use Chromium, Brave, Edge, or another tested Chromium-family browser, but the app target must remain deterministic.

## 8.1 Webapp app definition

Each webapp should have a simple config definition.

Canonical config directory:

```bash
/opt/tvbox-system/config/webapps
```

Config filename rule:

```bash
/opt/tvbox-system/config/webapps/<app_id>.conf
```
...
app_id=<stable id>
name=<display name>
browser=<browser command or explicit supported browser alias>
url=<URL or local file URL>
profile=<dedicated browser profile path>
input_profile=<tvbox-inputctl profile>
icon=<repo icon path>
mode=app|kiosk|fullscreen
```
...
start_fullscreen=0|1
kiosk=0|1
ozone_platform=wayland|x11|auto
extra_arg=<additional browser argument; repeatable if parser supports it>
allow_browser_fallback=0|1
fallback_browser=<browser command; repeatable if parser supports it>
```

Example: YouTube in Chromium:

```ini
app_id=youtube-chromium
...
mode=app
start_fullscreen=1
user_agent=Mozilla/5.0 (SMART-TV; Linux; Tizen 7.0) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/5.0 Chrome/108.0.0.0 Safari/537.36
allow_browser_fallback=0
```

Example: YouTube in Brave:

```ini
app_id=youtube-brave
name=YouTube Brave
browser=brave-browser
url=https://www.youtube.com/tv
profile=/home/tvbox/.config/brave-tvbox-youtube
input_profile=youtube_remote
icon=/opt/tvbox-system/assets/icons/youtube-brave.png
mode=app
start_fullscreen=1
user_agent=Mozilla/5.0 (SMART-TV; Linux; Tizen 7.0) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/5.0 Chrome/108.0.0.0 Safari/537.36
allow_browser_fallback=0
...
TVBox should borrow the config-driven launcher idea from the PiKVM launcher project, but not its default portability-first browser selection behavior.

Policy:

```text
Each webapp config pins its browser.
Runtime behavior must not silently change because a new browser was installed.
If the configured browser is missing, fail loudly, log the error, and return to Kodi.
Browser fallback is allowed only when allow_browser_fallback=1 is explicitly set for that webapp.
```

Reason:

```text
TVBox is an appliance.
Stability and reproducibility matter more than automatically finding any browser that happens to be installed.
```

Supported browser command examples:
...
Optional diagnostic/install behavior may still scan available browsers, but normal launch behavior should use the configured browser.

## 8.3 Generic launch command

Desired command shape:

```bash
tvboxctl launch webapp youtube-chromium
tvboxctl launch webapp youtube-brave
tvboxctl launch webapp fireboy-watergirl
tvboxctl launch webapp fixitfelix
```
...
Read the requested webapp config.
Validate required fields.
Resolve the configured browser command.
Fail loudly if the configured browser is unavailable and fallback is disabled.
Close local apps except target.
Set input profile from the webapp config.
Launch the configured browser with the configured profile and URL.
Focus the browser window.
Set active-context=webapp:<app_id>.
```

## 8.4 Browser launch requirements

Each webapp should use:

```text
Dedicated user data/profile directory.
App/kiosk/fullscreen style if appropriate.
Narrow process matching by profile path.
No shared profile between webapps.
No broad browser kills.
```

Possible Chromium command pattern:
...
Possible Brave command pattern:

```bash
brave-browser \
  --user-data-dir=/home/tvbox/.config/brave-tvbox-APPID \
  --no-first-run \
...
If a `user_agent=` field exists, add:

```bash
--user-agent="$USER_AGENT"
```

Exact flags should be tested per browser and per app.

## 8.5 Compatibility wrapper policy

Compatibility wrappers may exist for remote buttons or Kodi add-ons, but they should be tiny shims into the generic webapp command.

Example:

```bash
#!/bin/bash
exec /usr/local/bin/tvboxctl launch webapp youtube-chromium "$@"
```
...
```bash
#!/bin/bash
exec /usr/local/bin/tvboxctl launch webapp youtube-brave "$@"
```

Temporary one-off scripts such as `/usr/local/bin/tvbox-youtube-brave` are acceptable for testing, but the long-term implementation should move duplicated lifecycle logic into `tvboxctl` / `tvbox-webapp`.
...
Red C               F7  -> tvboxctl launch webapp <preferred-youtube-webapp>
...
/usr/local/bin/tvbox-plex
/usr/local/bin/tvbox-youtube
/usr/local/bin/tvbox-youtube-brave  # optional compatibility shim / test path
/usr/local/bin/tvbox-webapp
/usr/local/bin/tvbox-steamlink
```
...
Webapp                  Close active webapp, show Kodi
...
V1.3 should keep `EXIT_CLOSES_KODI=0` by default.
...
YouTube webapp        Navigate/relaunch YouTube TV home if reliable
Webapp/game           App-specific; often no-op or browser/app menu
...
│   ├── plugin.program.tvbox.youtube.brave/
...
close_webapp(app_id)
close_all_webapps_except(app_id)
show_kodi
close_kodi
```
...
Start with YouTube Chromium and YouTube Brave as separate webapp definitions.
...
tvboxctl menu
tvboxctl launch plex
tvboxctl launch webapp <preferred-youtube-webapp>
```
...
Launch webapp game from Kodi add-on
...
```text
Do not broad-kill browsers. Match webapps by configured profile path.
Do not broad-kill shell.
Do not kill Sunshine except from explicit hard Moonlight Exit behavior.
Do not let local external apps remain behind Kodi in V1.3.
...
YouTube launches as one or more configured webapps and Home reliably closes the active one.
At least one controller remapping profile works for a webapp.
...
F12 stopped working from Steam Link, desktop, Firefox, webapps, and other non-Kodi contexts because the labwc global keybind was gone.
```
...
V1.3 should include a repair/deploy script for labwc config.
...
F12 from YouTube/webapp -> active webapp closes and Kodi returns
```
